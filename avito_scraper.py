...
def select_first(element, selector_key: str, attr: str = None):
    ...
    for selector in selectors:
        try:
            ...
            if result:
                logger.debug(f"Селектор '{selector}' сработал для '{selector_key}'")
                if attr:
...
def select_all(element, selector_key: str) -> list:
    ...
    for selector in selectors:
        try:
            ...
            if results:
                logger.debug(f"Селектор '{selector}' нашёл {len(results)} элементов для '{selector_key}'")
                return results
        except Exception:
            continue
...
def collect_listings(page, search_url: str, price_min: int) -> list:
    """
    ...
    with Progress(
        ...
        while current_page <= MAX_PAGES:
            ...
            logger.debug(f"Найдено {len(cards)} карточек на странице {current_page}")
            ...
            if not page_listings:
                logger.info(f"Страница {current_page}: нет подходящих карточек, завершаем пагинацию")
                break
            
            all_listings.extend(page_listings)
            logger.info(f"Страница {current_page}: добавлено {len(page_listings)} карточек (всего: {len(all_listings)})")
    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                ...
                listings = collect_listings(page, search_url, price_min)
